...
class Tintuc extends NIW_Controller
{
	...
	function page($index=0)
	{
		...
		$data['items']=$this->Mtintuc->getListByColumn('ta_news','news_title',1);
		$data['title']='tienganh | Tin tức';
		$data['list_tintuc']=$this->Mtintuc->getListOffset('ta_news',8,$index);
	...
	function detail($alias)
		{
			...
			//$data['product']=$this->Mtintuc->getListOffset('ln_product',5,0);
			$data['news']=$this->Mtintuc->getRowByColumn('ta_news','news_id',$news_id);
			$model=new CI_Model();
			$data['module']=$this->module;
			$data['page']='vdetail';
			$this->load->view('front/container',$data);
