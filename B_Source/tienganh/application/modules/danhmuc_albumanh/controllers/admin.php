 ...
 */
	class Admin extends Admin_Controller
	{
		...
		function __construct()
		{
			...
			$this->load->model('Mdanhmuc_albumanh');
		...
		{
			$config['base_url']=base_url().'danhmuc_albumanh/admin/page/';
			$config['per_page']=10;
			$config['total_rows']=count($this->Mdanhmuc_albumanh->getListFull('ta_albums_category'));
			$config['uri_segment']=4;
			$this->pagination->initialize($config);
			
			$data['title']='Danh mục';
			$data['bcCurrent']='danh mục';
			$data['list']=$this->Mdanhmuc_albumanh->getListOffset('ta_albums_category',10,$index);
		...
		function insert()
		{
			...
			{
				$data['list']=$this->Mdanhmuc_albumanh->getListFull('ta_albums_category');
			...
			else 
			{
				...
				{
					$input=$this->_input();
					if ($this->Mdanhmuc_albumanh->insertNewRow('ta_albums_category',$input))
					{
		...
		function edit($id=0)
		{
			...
			if (!$this->input->post('category_name'))
			{
				$data['info']=$this->Mdanhmuc_albumanh->getRowByColumn('ta_albums_category','id',$id);
			...
			else 
			{
				...
				{
					$input=$this->_input();
					if ($this->Mdanhmuc_albumanh->updateRowByColumn('ta_albums_category','id',$id,$input))
					{
		...
		{
			if ($this->Mdanhmuc_albumanh->deleteRowByColumn('ta_albums_category','id',$id))
			{
