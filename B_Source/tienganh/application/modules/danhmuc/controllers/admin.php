 ...
 */
	class Admin extends Admin_Controller
	{
		...
		function page($index=0)
		{
			...
			$config['per_page']=10;
			$config['total_rows']=count($this->Mdanhmuc->getListFull('ta_courses_cate'));
			$config['uri_segment']=4;
			$this->pagination->initialize($config);
			
			$data['title']='Danh mục';
			$data['bcCurrent']='danh mục';
			$data['list']=$this->Mdanhmuc->getListOffset('ta_courses_cate',10,$index);
		...
		function insert()
		{
			...
			{
				$data['list']=$this->Mdanhmuc->getListFull('ta_courses_cate');
			...
			else 
			{
				...
				{
					$input=$this->_input();
					if ($this->Mdanhmuc->insertNewRow('ta_courses_cate',$input))
					{
		...
		function edit($id=0)
		{
			...
			if (!$this->input->post('cate_name'))
			{
				$data['info']=$this->Mdanhmuc->getRowByColumn('ta_courses_cate','id',$id);
			...
			else 
			{
				...
				{
					$input=$this->_input();
					if ($this->Mdanhmuc->updateRowByColumn('ta_courses_cate','id',$id,$input))
					{
		...
		{
			if ($this->Mdanhmuc->deleteRowByColumn('ta_courses_cate','id',$id))
			{
