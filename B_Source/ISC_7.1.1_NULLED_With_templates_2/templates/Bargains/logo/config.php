...
class Bargains_logo extends LogoMaker
{
	/**
	 * TextFieldCount
	...
	*/
	...
	public function GenerateLogo()
	{
		...
		$t_box = $this->TextBox($this->Text[0], 'VAGRoundedStd-Bold.otf', '', 44, 0, 0);
		...
		if(strlen($this->Text[0]) > 0) {
			...
			$text_position = $this->AddText($this->Text[0], 'VAGRoundedStd-Bold.otf', '', 40, 10, $y_pos-1);
			$text_position = $this->AddText($this->Text[0], 'VAGRoundedStd-Bold.otf', '', 40, 9, $y_pos);

			$text_position = $this->AddText($this->Text[0], 'VAGRoundedStd-Bold.otf', '5a5a5a', 40, 11, $y_pos);
		...
		if(strlen($this->Text[1]) > 0) {
			...
			$text_position2 = $this->AddText($this->Text[1], 'MrSandsfort.otf', 'd00400', 34, $top_right+15, $y_pos+8);
			$top_right = $text_position2['top_right_x'];
		}
		$this->SetImageSize($top_right+20, 65);
