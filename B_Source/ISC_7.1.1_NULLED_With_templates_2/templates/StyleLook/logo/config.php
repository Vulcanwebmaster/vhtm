...
class StyleLook_logo extends LogoMaker
{
	/**
	 * TextFieldCount
	...
	*/
	...
	public function GenerateLogo()
	{
		...
		$t_box = $this->TextBox($this->Text[0], 'Annabelle.ttf', '000103', $textSize, 0, 0);
		...
		if(isset($this->Text[0]) && strlen($this->Text[0]) > 0) {
			...
			$text_position = $this->AddText($this->Text[0], 'Annabelle.ttf', 'FFC100',$textSize, $textLeft, 20);
			$top_right = $text_position['top_right_x'];
		}

		if(isset($this->Text[1]) && strlen($this->Text[1]) > 0) {
			// AddText() - text, font, fontcolor, fontSize (pt), x, y, center on this width
			$text_position2 = $this->AddText($this->Text[1], 'trebuc.ttf', 'FFFFFF',$textSize, ($text_position['top_right_x']), 20);
