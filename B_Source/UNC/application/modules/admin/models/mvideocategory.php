<?php
	class Admin_Model_Mvideocategory extends Zend_Db_Table_Abstract
	{
		private $db;
		...
		function getListCM()
		{
			$query=$this->db->query('select * from unc_video_category order by is_active desc, category_id desc');
		...
		{
			$query=$this->db->query('select * from unc_video_category where category_parent_id=0');
			return $query->fetchAll();
		}
		
		function deleteCm($id)
		{
			$query=$this->db->query('delete from unc_video_category where category_id="'.$id.'"');
			return $query;
		}
		
		function insertCm($input)
		{
			$query=$this->db->query('insert into unc_video_category values ("","'.$input['category_name'].'","'.$input['alias'].'","'.$input['is_active'].'")');
			return $query;
		}
		
		function getCategoryNameById($id)
		{
			$query=$this->db->query('select category_name from unc_video_category where category_id="'.$id.'"');
			$list=$query->fetchAll();
			return $list[0];
		}
		...
		{
			$query = $this->db->query('select * from unc_video_category where category_id="'.$id.'" and category_parent_id=0');
			$list = $query->fetchAll();
			if(count($list)>0)
				return true;
			else return false;
		...
		{
			$query=$this->db->query('update unc_video_category 
