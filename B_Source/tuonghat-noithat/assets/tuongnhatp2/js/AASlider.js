...
Note:
        ...
        Aqui tuve que meter mano (CarlosMC) porque tenia algun bug (hay alguna cosa a machete)
Arguments:
...
//			this.maxknob.setStyle('position', 'relative').setStyle(this.p, + this.max - this.options.offset).setStyle('bottom', this.options.knobheight);			
			this.knob.setStyle('position', 'relative').setStyle(this.p, this.options.maxoffset);
		}
		var lim = {};
		//status = this.z
...
//		this.bkg.style.clip = "rect(0px "+  (parseInt(this.toPosition(this.step)) +3) + "px 10px 0px)";
		// Itx: Aqui meto mano yo
		if(this.maxknob && this.p == 'top') {
			this.bkg.style.clip = "rect(" + stepMin + " 1px " + this.maxknob.getStyle(this.p) + " 0px)";
		} else {
			this.bkg.style.clip = "rect(0px "+  (parseInt(this.toPosition(this.step)) +3) + "px 10px 0px)";
		}		
...
//			mx= parseInt(this.maxknob.getStyle('left')) - this.options.offset-4 ;
			...
			// Itx: Aqui meti mano yo
//			mx= parseInt(this.maxknob.getStyle(this.p)) - this.options.offset-4 ;
			mx= parseInt(this.maxknob.getStyle(this.p)) - this.stepSize;
			lim[this.z] = [mi, mx];
...
//			mi = parseInt(this.knob.getStyle('left'))-this.options.offset+22; 
			...
			// Itx: Aqui meti mano yo
//			mi = parseInt(this.knob.getStyle(this.p))-this.options.offset+22;
			mi = parseInt(this.knob.getStyle(this.p)) + this.stepSize;
...
//			this.clip_l = parseInt(this.knob.getStyle('left')) + 10;
			//Itx: Aqui meti mano yo
			this.clip_l = parseInt(this.knob.getStyle(this.p)) + 10;
...
//			var w = Math.abs(parseInt(this.knob.getStyle('left')) - parseInt(this.maxknob.getStyle('left')));
			var w = Math.abs(parseInt(this.knob.getStyle(this.p)) - parseInt(this.maxknob.getStyle(this.p)));
			//if(w > 3) w = w+3;
			
			var r = parseInt(this.clip_l + w);
			// Itx: Aqui meti mano yo
			if (this.p == 'top') {				
				this.bkg.style.clip = "rect(" + this.knob.getStyle(this.p) + " 1px " + this.maxknob.getStyle(this.p) + " 0px)";
			} else {			
				this.bkg.style.clip = "rect(0px "+  r + "px 10px "+ this.clip_l + "px)";
			}						
...
AASlider.implement(new Events);
AASlider.implement(new Options);