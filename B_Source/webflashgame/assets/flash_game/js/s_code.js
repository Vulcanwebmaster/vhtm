...
var s_account = s_account || 'greendev2',
s_internalFilterSet = s_internalFilterSet || '',
s = s_gi(s_account);
...
s.linkInternalFilters = 'javascript:' + s_internalFilterSet;
s.linkLeaveQueryString = false;
s.linkTrackVars = 'None';
s.linkTrackEvents ='None';
s.dstStart = '3/25/2012';
s.dstEnd = '10/28/2012';
s.currentYear = '2012';
s.server = location.hostname;
s.visitorNamespace = 'greentubeiesgmbh';
s.trackingServer = 'greentubeiesgmbh.d3.sc.omtrdc.net';
...
function s_doPlugins(s) 
{
    ...
    s.prop11=s.eVar11=s.getTimeParting('h','+1');
    s.prop12=s.eVar12=s.getTimeParting('d','+1');
    s.prop13=s.eVar13=s.getTimeParting('w','+1');

    s.prop14=s.eVar14=s.getDaysSinceLastVisit();
    ...
    if(!s.eVar27){
        s.eVar27=s.getQueryParam('ctid');
        s.eVar27=s.getValOnce(s.eVar27,'s_evar_27',0);
    }

    if(!s.eVar26){
        s.eVar26=s.getQueryParam('afid');
        s.eVar26=s.getValOnce(s.eVar26,'s_evar_26',0);
    }

    if(!s.eVar15){
        s.eVar15=s.getQueryParam('kw');
        s.eVar15=s.getValOnce(s.eVar15,'s_evar_15',0); 
    }

    if(!s.eVar16){
        s.eVar16=s.getQueryParam('match');
        s.eVar16=s.getValOnce(s.eVar16,'s_evar_16',0); 
    }

    if(!s.eVar36){
        s.eVar36=s.getQueryParam('cre');
        s.eVar36=s.getValOnce(s.eVar36,'s_evar_36',0); 
    }

    if(!s.eVar37){
        s.eVar37=s.getQueryParam('plc');
        s.eVar37=s.getValOnce(s.eVar37,'s_evar_37',0); 
    }

    if(!s.eVar38){
        s.eVar38=s.getQueryParam('mtt');
        s.eVar38=s.getValOnce(s.eVar38,'s_evar_38',0); 
    }

    if(!s.eVar39){
        s.eVar39=s.getQueryParam('network');
        s.eVar39=s.getValOnce(s.eVar39,'s_evar_39',0); 
    }

    if(!s.eVar41){
        s.eVar41=s.getQueryParam('camp');
        s.eVar41=s.getValOnce(s.eVar41,'s_evar_41',0); 
    }
}
