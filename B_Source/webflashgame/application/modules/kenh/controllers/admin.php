 ...
 */
	class Admin extends Admin_Controller
	{
		...
		{
			$config['base_url']=base_url().'kenh/admin/page/';
			$config['per_page']=10;
			$config['total_rows']=count($this->Mkenh->getListFull('fg_channel'));
			$config['uri_segment']=4;
			$this->pagination->initialize($config);
			
			$data['title']='Kênh';
			$data['bcCurrent']='kênh';
			$data['list']=$this->Mkenh->getListOffset('fg_channel',10,$index);
		...
		function insert()
		{
			...
			{
				$data['list']=$this->Mkenh->getListFull('fg_channel');
				$data['config'] = $this->setupCKEditor('97%','200px');
				$data['title']='Thêm Kênh';
				$data['bcCurrent']='Kênh';
				$data['module']=$this->module;
			...
			else 
			{
				...
				{
					$input=$this->_input();
					if ($this->Mkenh->insertNewRow('fg_channel',$input))
					{
		...
		function edit($id=0)
		{
			...
			if (!$this->input->post('name'))
			{
				$data['info']=$this->Mkenh->getRowByColumn('fg_channel','id',$id);
				$data['title']='Sửa kênh';
				$data['bcCurrent']='kênh';
				$data['module']=$this->module;
			...
			else 
			{
				...
				{
					$input=$this->_input();
					if ($this->Mkenh->updateRowByColumn('fg_channel','id',$id,$input))
					{
		...
		{
			if ($this->Mkenh->deleteRowByColumn('fg_channel','id',$id))
			{
