	...
	<form target="paymentpopup" action="https://www.paypal.com/cgi-bin/webscr" method="post" name="frm_popup_payment" id="frm_popup_payment">
		<table class="formTable">
			...
			<tbody>
				<tr>
					<td>
						<input type="hidden" name="item_number" value="<?php echo $_SESSION['currencygame_123'] ?> Twists">	<!-- Phần này chính là phần sẽ hiển thị mô tả về sản phẩm của bạn thường là mã code phân loại sản phẩm của bạn -->
						...
						<input type="hidden" name="currency_code" 
							<?php if(isset($currency_setting->currency)){ ?>
								value="<?php echo $currency_setting->currency ?>"
							<?php }else{ ?>
								value="EUR"
							<?php } ?>>		<!-- Phần này chính là mã tiền tệ mà bạn muốn thanh toán EUR, USD... -->
						...
						<input type="hidden" name="item_name" value="<?php echo $_SESSION['currencygame_123'] ?> Twists" id="item_name"  size="45"> <!-- Phần này chính là phần ghi chú từ phía người mua, đây chính là phần sẽ được ghi vào invoice rằng họ đã mua cái gì. -->
						<input type="hidden" name="amount"  value="<?php echo $_SESSION['chuyentiennao'] ?>" id="amount" size="45"> <!-- Giá trị tiền tệ cần để đổi 1$, 2$... -->
