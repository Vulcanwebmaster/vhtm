...
function changeStatus(obStatus)
	{
		var trading_id=document.getElementById('trading_id').value;
		var status=obStatus;
		
		var queryString="id="+trading_id+"&status="+status;
		
		getAJaxReqest();
		var url="ajaxfiles/updatetradestatus.php";
			...
			if(req.readyState==4){
				...
				if(result=="Updated Successfully."){
					...
					for(var i=0;i<JSONTrading.length;i++){
						if(trading_id==JSONTrading[i].id){
...
function pageselectCallbackCity(page_index, jq){
	...
	for(var i=page_index*items_per_page;i<max_elem;i++)
	{
		...
		newcontent+="<tr class='result' style='background-color:"+color+"; margin:10px;'>";
			...
			newcontent += '<td>' + JSONTrading[i].id_isin + '</td>';
...
function ViewTransfer(id)
{
	for(var i=0;i<JSONTrading.length;i++)
	{
		if(id==JSONTrading[i]['id'])
		{
			...
			document.getElementById('trading_id').value=JSONTrading[i].id;
			...
			$("#id_comment").text(JSONTrading[i].isin_reference);
			$("#id_fundname").text(JSONTrading[i].fund_name);
			...
			if(JSONTrading[i].status=="pending")
			{
				showButtonDialog('dialog','SETTLED',true);
				showButtonDialog('dialog','COMPLETED',true);
				showButtonDialog('dialog','CANCELLED',true);
			}
			else if(JSONTrading[i].status=="completed")
			{
				showButtonDialog('dialog','SETTLED',true);
				showButtonDialog('dialog','COMPLETED',false);
				showButtonDialog('dialog','CANCELLED',true);
			}
			else if(JSONTrading[i].status=="settled" || JSONTrading[i].status=="cancelled")
			{
				showButtonDialog('dialog','SETTLED',false);
				showButtonDialog('dialog','COMPLETED',false);
				showButtonDialog('dialog','CANCELLED',false);
