...
function newAppointment(ob)
{
	...
	showButtonDialog('appointment_entry','APPROVE',false);
...
function showNewAppointment()
{
	...
	for(var i=0;i<JSONAppointments.length;i++)
	{
		...
		if((dt>=temp1 && dt<=temp2) || (dt==temp1) || (dt==temp2)) {
			...
			showButtonDialog('appointment_entry','APPROVE',true);
			break;
		}
	}
...
function APPROVE()
{
	...
	var url="ajaxfiles/approve_appointment_cal_ajax.php";
	req=null;
	getAJaxReqest();
			...
			if(result=="Approved.")
			{
				...
				for(var i=0;i<JSONAppointments.length;i++){
						if(appointment_id==JSONAppointments[i].appointmentid){
							...
							var tt1=parseInt(tmpArr1[2].toString());
							var tt2=parseInt(tmpArr2[2].toString());
							//alert(tt1+""+tt2);
							for(var i=tt1;i<=tt2;i++)
							{
								var divTemp="div"+i;
								document.getElementById(divTemp).className="approveddate";
