...
$(function() {
		...
		var firstname = $("#firstname"),					lastname = $("#lastname"),
			...
			clientid=$('#clientid'),
			active=$('#active'),
			allFields = $([]).add(firstname).add(lastname).add(email).add(password).add(loginid).add(role).add(id).add(gender).add(company).add(phone).add(department).add(active).add(clientid),
			tips = $("#validateTips");
			...
			buttons: {
				'Save': function() {
					...
					if (bValid) {						getAJaxReqest();
						...
						queryString+="&gender="+gender.val();
						queryString+="&company="+company.val();
...
function ViewUser(id1)
{
	for(var i=0;i<JSONUser.length;i++)
	{
		if(id1==JSONUser[i].id)
		{
			...
			setValueFromId('firstname',JSONUser[i].firstname);									setValueFromId('lastname',JSONUser[i].lastname);						setValueFromId('department',JSONUser[i].officephone);
			setValueFromId('gender',JSONUser[i].gender);						
			setValueFromId('company',JSONUser[i].company);									setValueFromId('email',JSONUser[i].email);						alert(JSONUser[i].id);						
