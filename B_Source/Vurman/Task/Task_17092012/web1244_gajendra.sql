...
--
-- Host: localhost
-- Generation Time: Sep 17, 2012 at 06:43 PM
...
--
-- Table structure for table `isin_currency_view`
--
-- Hoang comment CREATE ALGORITHM because dispaly on server
--CREATE ALGORITHM=UNDEFINED DEFINER=`web1244`@`localhost` SQL SECURITY DEFINER VIEW `web1244_gajendra`.`isin_currency_view` AS select `web1244_gajendra`.`isin`.`id` AS `id`,`web1244_gajendra`.`isin`.`code` AS `code`,`web1244_gajendra`.`isin`.`name` AS `name`,`web1244_gajendra`.`isin`.`id_currency` AS `id_currency`,`web1244_gajendra`.`isin`.`limit1` AS `limit1`,`web1244_gajendra`.`isin`.`limit2` AS `limit2`,`web1244_gajendra`.`isin`.`limit3` AS `limit3`,`web1244_gajendra`.`isin`.`name_fund` AS `name_fund`,`web1244_gajendra`.`isin`.`nav` AS `nav`,`web1244_gajendra`.`isin`.`date_nav` AS `date_nav`,`web1244_gajendra`.`isin`.`unique_id` AS `unique_id`,`web1244_gajendra`.`currency`.`name` AS `currency_name`,`web1244_gajendra`.`currency`.`abbreviation` AS `abbreviation` from (`web1244_gajendra`.`isin` left join `web1244_gajendra`.`currency` on((`web1244_gajendra`.`isin`.`id_currency` = `web1244_gajendra`.`currency`.`id`)));
-- in use (#1356 - View 'web1244_gajendra.isin_currency_view' references invalid table(s) or column(s) or function(s) or definer/invoker of view lack rights to use them)
...
(11, 1, 'vuj', 'IE.003''077''186''3', 'POALIM ASSET GLOBAL MULTI-MANAGER 20 USD', '', 1, 100, 1, '2012-05-29', '2012-06-01', '', 'BANK XYZ', '', '', 'custodian xyz', '', '', '', '', '', '', '2012-05-29 21:35:23', 'incoming', 'pending', 'receive', NULL, NULL),
(10, 1, 'vuj', 'IE.003''077''186''3', 'POALIM ASSET GLOBAL MULTI-MANAGER 20 USD', '', 1, 100, 1, '2012-05-17', '2012-05-22', '2', 'BANK XYZ', 'BXYZ1234XXX', '11', 'CUSTODIAN ABC', 'CABC0987XXX', '20.123.456', 'TA ABC', 'TAABC456XXX', '1881', '', '2012-05-17 13:58:06', 'incoming', 'pending', 'receive', NULL, NULL),
...
(24, 1, 'vuj', 'IE.003''077''186''3', 'POALIM ASSET GLOBAL MULTI-MANAGER 20 USD', '', 1, 100, 2, '2012-09-17', '2012-09-20', '3', 'ADLER & CO. AG', 'ADCOCHZZXXX', '100', 'REICHMUTH & CO.', 'REICCH22XXX', 'R49610900', 'TA ABC', 'TAABC456XXX', '', '', '2012-09-17 17:12:44', 'incoming', 'pending', 'receive', NULL, NULL);
...
INSERT INTO `user` (`id`, `loginid`, `password`, `email`, `officephone`, `cellphone`, `phone`, `firstname`, `lastname`, `department`, `role`, `active`, `clientid`) VALUES
(1, '1', '2', '', '', '', '', 'November', 'ADLER & CO. AG', 'SETTLEMENT', 'user', '0', 1),
