<?php
	//============================== SLIDE ====================================
	$lang['slide-text']='Chào mừng đến với website chính thức của Tường Nhật';
	//============================== MENU ====================================
	$lang['menu-trangchu']='Trang chủ';
	$lang['menu-sanpham']='Sản phẩm';
	$lang['menu-giaohang']='Giao hàng';
	$lang['menu-dichvu']='Dịch vụ & Khuyến mãi';
	$lang['menu-moi']='Mới';
	$lang['menu-dhe']='Dhe';
	$lang['menu-lienhe']='Liên hệ';
	$lang['menu-b2b']='b2b';
	$lang['menu-tuyendung']='Tuyển dụng';
	//============================== HEADER ====================================
	$lang['header-dieukhoan']='Điều khoản';
	$lang['header-cauhoi']='FAQ';
	$lang['header-nhasanxuat']='Nhà sản xuất';
	$lang['header-dangnhap']='Đăng nhập';
	$lang['header-tim']='Tìm';
	$lang['header-timkiem']='Tìm kiếm';	
	//============================== LIEN HE ====================================
	$lang['form_lien_he']='Thông tin liên hệ';
	$lang['lienhe_hoten']='Họ tên';
	$lang['lienhe_congty']='Công ty';
	$lang['lienhe_makhachhang']='Mã khách hàng';
	$lang['lienhe_pho']='Phố';
	$lang['lienhe_thanhpho']='Postcode, Thành phố';
	$lang['lienhe_email']='E-Mail';
	$lang['lienhe_dt']='Điện thoại';
	$lang['lienhe_yeucaulienhe']='Yêu cầu liên hệ';
	$lang['lienhe_yeucau1']='Tôi là khách hàng mới';
	$lang['lienhe_yeucau2']='Hãy gửi cho tôi bản sao danh mục của bạn';
	$lang['lienhe_yeucau3']='Hãy gọi cho tôi';
	$lang['lienhe_yeucau4']='Tôi muốn đặt mật khẩu cho các trang sản phẩm của bạn';
	$lang['lienhe_yeucau5']='Request for exhibition appointment';
	$lang['lienhe_yeucau6']='Newsletter order';
	$lang['lienhe_yeuthich']='Tôi yêu thích';
	$lang['lienhe_yeuthich1']='Các sản phẩm bổ sung cho bánh';
	$lang['lienhe_yeuthich2']='Tự phục vụ các loại bánh';
	$lang['lienhe_cauhoi']='Chú thích hoặc câu hỏi';
	$lang['lienhe_ma']='Mã bảo vệ';
	$lang['lienhe_nhapma']='Hãy nhập đoạn mã trong hình trên';
	$lang['lienhe_submit']='Liên hệ';
	//============================== DANH MUC SAN PHAM ====================================
	$lang['category-admin-product-category']='Danh mục sản phẩm';
	$lang['category-admin-id']='Mã';
	$lang['category-admin-name-vn']='Tên (t. Việt)';
	$lang['category-admin-name-en']='Tên (t. Anh)';
	$lang['category-admin-description-vn']='Mô tả (t. Việt)';
	$lang['category-admin-description-en']='Mô tả (t. Anh)';
	$lang['category-admin-image']='Ảnh';
	$lang['category-admin-actions']='Thao tác';
	$lang['category-admin-insert']='Tạo mới';
	$lang['category-admin-insert-new']='Tạo mới danh mục sản phẩm';
	$lang['category-admin-update']='Cập nhật';
	$lang['category-admin-update-category']='Cập nhật danh mục sản phẩm';
?>