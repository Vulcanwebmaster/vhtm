<?php
	//============= MENU ==================
	$lang['menu-trangchu']='Trang chủ';
	$lang['menu-gioithieu']='Giới thiệu';
	$lang['menu-sanpham']='Sản phẩm';
	$lang['menu-dichvu']='Dịch vụ';
	$lang['menu-lienhe']='Liên hệ';
	$lang['menu-tintuc']='Tin tức';
	
	$lang['header-hientai']='Hiện tại';
	$lang['header-ngonngu']='Ngôn ngữ';
	$lang['header-giohang']='Giỏ hàng';
	
	//============= LEFT ==================
	$lang['left-danhmuc']='Danh mục sản phẩm';
	$lang['left-doitac']='Đối tác';
	$lang['right-tinmoi']='Tin mới';
	
	//============== RIGHT ================
	$lang['right-hotro']='Hỗ trợ online';
	$lang['right-tinmoi']='Tin mới';
	$lang['right-docthem']='Xem tiếp';
	
	//============= FOOTER ================
	$lang['footer-trangchu']='Trang chủ';
	$lang['footer-sanphammoi']='Sản phẩm mới';
	$lang['footer-uudai']='Ưu đãi đặc biệt';
	$lang['footer-taikhoan']='Tài khoản của tôi';
	$lang['footer-diachi']='Địa chỉ';
	$lang['footer-lienhe']='Liên hệ';
	
	//============ CONTENT ================
	$lang['content-gioithieu']='Giới thiệu';
	$lang['content-dichvu']='Dịch vụ';
	$lang['content-lienhe']='Liên hệ';
	$lang['content-sanpham']='Sản phẩm';
	$lang['content-tintuc']='Tin tức';
	$lang['content-sanphammoi']='Sản phẩm mới';
	$lang['content-trangchu']='Trang chủ';
	$lang['content-thongso']='Thông số kỹ thuật';
	$lang['content-gia']='Giá';
	$lang['right-docthem']='Xem tiếp';
	
	//=========== CONTACT US =============
	$lang['contact-tendaydu']='Tên đầy đủ';
	$lang['contact-email']='Địa chỉ email';
	$lang['contact-tencongty']='Tên công ty';
	$lang['contact-sodienthoai']='Số điện thoại';
	$lang['contact-madonhang']='Mã số đơn hàng';
	$lang['contact-maRMA']='Mã RMA';
	$lang['contact-chitiet']='Các chi tiết';
?>