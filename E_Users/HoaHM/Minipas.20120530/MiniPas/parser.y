...
const_declaration : INTEGER IDENTIFIER ASSGNOP NUMBER_VAL ';' { const_count++; install( $2 , "global", C_INT, -1); gen_code( LD_INT, $4 ); context_check( STORE, $2, function_name );  }
| CHAR IDENTIFIER ASSGNOP CHR_VAL ';' { const_count++; install( $2, function_name, C_CHR, -1 ); gen_code_char ( LD_CHR, $4); context_check( STORE, $2, function_name );  }
| DOUBLE IDENTIFIER ASSGNOP NUMBERD_VAL ';' { const_count++; install( $2, function_name, C_DOU, -1 ); gen_code_double( LD_DOU, $4 ); context_check( STORE, $2, function_name );  }
| STRING  IDENTIFIER ASSGNOP STR_VAL ';' { const_count++; install( $2, function_name, C_STR, -1 ); gen_code_string ( LD_STR, $4); context_check( STORE, $2, function_name ); }
| BOOLEAN IDENTIFIER ASSGNOP NUMBERB_VAL ';' { const_count++; install( $2, function_name, C_BOL, -1 );  gen_code_boolean (LD_BOL, $4); context_check( STORE, $2, function_name);}
;
