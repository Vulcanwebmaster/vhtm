		...
		switch (ir.op) 
		{
			...
			case READ_VAR :
				...
				if (ir.arg.type == INT)
				{
					...
					{
						yyerror("Loi input kieu integer sai");
						return;
					}
					else
					{					
				...
				if (ir.arg.type == DOU)
				{
					...
					{
						yyerror("Loi input kieu double sai");
						return;
					}
					else
					{					
				...
				if (ir.arg.type == BOL)
				{
					...
					if (strcmp(temp4,"true")) stack[top_index+ir.arg.int_val].bol_val = 1;
						...
						{
							yyerror("Loi input kieu boolean sai");
							return;
						}
				}
				else 
				{
					yyerror("Loi khong ho tro dang nhap nay");
					return;
				}
				break;
			case WRITE_STR : 
			...
			case WRITE_ALL :
				fflush(stdout);
				fflush(stdin);
				if (current_type == INT || current_type == C_INT)
				{
					printf("%ld",stack[top--].int_val);
				} else 
				if (current_type == DOU || current_type == C_DOU)
				{
					printf("%f",stack[top--].dou_val);
				} else 
				if (current_type == CHR || current_type == C_CHR)
				{
					printf("%c",stack[top--].chr_val);
				} else 
				if (current_type == STR || current_type == C_STR)
				{
					printf("%s",stack[top--].str_val);
				} else 
				if (current_type == BOL || current_type == C_BOL)
				{
			...
			case WRITE_LINE :
				...
				break;
			case STORE : 
				if (ir.arg.type == C_INT || ir.arg.type == C_DOU || ir.arg.type == C_CHR || ir.arg.type == C_STR || ir.arg.type == C_BOL)
				{
					const_count--;
					if (const_count < 0)
					{
						yyerror("Loi - Khong the gan gia tri cho hang so");
						return;
					}
				}
